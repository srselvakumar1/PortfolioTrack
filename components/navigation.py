...
import threading
import time
from components.ui_elements import sidebar_stats_card
...
class PremiumSidebar(ft.Container):
    ...
    def __init__(self, page: ft.Page, on_nav_change, toggle_sidebar_cb=None):
        ...
        self._logo_rotation = 0
        
        def animate_logo():
            while True:
                time.sleep(0.05)
                self._logo_rotation = (self._logo_rotation + 2) % 360
                try:
                    logo_icon.rotation = self._logo_rotation
                    logo_icon.update()
                except:
                    pass
