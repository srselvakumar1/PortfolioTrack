...
from flet_app.components.ui_elements import page_title, premium_card, info_metric, status_chip, dashboard_breakdown_header, dashboard_broker_stats, dashboard_broker_row_item
from flet_app.common.engine import get_dashboard_metrics, get_metrics_by_broker, get_top_worst_performers, get_actionable_insights, get_tax_harvesting_opportunities, fetch_and_update_market_data
...
class DashboardView(ft.Container):
    def __init__(self, app_state: AppState):
        ...
        self._latest_payload = None  # (metrics, performers, insights, harvesting)
    ...
    def did_mount(self):
        ...
        # If background work completed while we were hidden, apply it now.
        if self._pending_render and self._latest_payload is not None:
            self._pending_render = False
            try:
                metrics, performers, insights, harvesting = self._latest_payload
                self._apply_payload(metrics, performers, insights, harvesting)
            except Exception:
                pass
        # If someone requested a load while hidden, run it now.
    ...
    def did_unmount(self):
        self._is_active = False
        # Avoid leaving overlays visible when navigating away
        try:
            self.popover_container.visible = False
            self.backdrop.visible = False
        except Exception:
    ...
    def _apply_payload(self, metrics, performers, insights, harvesting):
        """UI thread: apply a fetched dashboard payload to existing controls."""
        ...
        try:
            from flet_app.components.navigation import PremiumSidebar
    ...
    def _fetch_and_render(self):
        ...
        # Store latest payload so we can apply it when active.
        self._latest_payload = (metrics, performers, insights, harvesting)
    ...
    async def finish_refresh(self):
        ...
        if not getattr(self, "_is_active", False):
            # Defer reload/UI update until user navigates back to Dashboard
            self._pending_load = True
            self._pending_render = True
            return
        self.load_data()
