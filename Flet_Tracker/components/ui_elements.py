...
def show_toast(page: ft.Page, message: str, color: str = ft.Colors.BLUE, duration_ms: int = 2000):
    ...
    Uses page.open() for modern Flet API (avoids full page.update()).
    Cleans up stale SnackBars from overlay to prevent unbounded growth.
    """
    # Clean up closed SnackBars from previous toasts to prevent overlay bloat
    page.overlay[:] = [c for c in page.overlay if not (isinstance(c, ft.SnackBar) and not getattr(c, 'open', False))]
...
def enhanced_filter_panel(broker_dropdown, symbol_input, iv_dropdown, exclude_checkbox, apply_btn, clear_btn, refresh_btn=None, active_count: int = 0) -> ft.Container:
    ...
    return ft.Container(
        content=ft.Row([
            ft.Text("FILTERS", size=10, weight=ft.FontWeight.W_700, color=ft.Colors.BLUE_300),
            filter_badge,
...
def holdings_view_header() -> ft.Container:
    """Compact view header — single row (Improvement #12)"""
    return ft.Container(
        content=ft.Row([
            ft.Text("Holdings", size=18, weight=ft.FontWeight.W_800, color=ft.Colors.WHITE),
            ft.Text("📊", size=16),
