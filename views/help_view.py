...
class HelpView(ft.Container):
    ...
    def _create_example_table(self, columns, rows_data):
        cols = [ft.DataColumn(ft.Text(c, color=self.text_secondary, weight=ft.FontWeight.BOLD, size=12)) for c in columns]
        
        rows = []
        for row in rows_data:
            cells = []
            for item in row:
                cells.append(ft.DataCell(ft.Text(str(item), color="#FFFFFF", size=12)))
