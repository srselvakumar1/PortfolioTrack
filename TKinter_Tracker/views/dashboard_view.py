...
class DashboardView(BaseView):
    ...
    def build(self):
        ...
        def _section_card(parent, title: str, icon: str, accent: str) -> tuple[tk.Frame, tk.Frame]:
            ...
            body_outer = tk.Frame(card, bg=ModernStyle.BG_SECONDARY, height=ModernStyle.DASH_SECTION_CARD_HEIGHT)
        ...
        self.harvest_card, self.harvest_body = _section_card(grid, "Tax Harvesting Options", "🏦", "#7C3AED")
    ...
    def _open_trade_drilldown(self, symbol: str, *, broker: str | None = None) -> None:
        ...
        for c, w in zip(cols, widths):
            ...
            tv.column(c, width=w, anchor="w" if c != "Action" else "center")
    ...
    def _show_broker_breakdown(self, title: str, metric_key: str, *, is_currency: bool = True) -> None:
        ...
        for idx, (broker, v) in enumerate(rows):
            ...
            tv.insert("", "end", iid=broker, values=(broker, disp, f"{pct:.1f}%"), tags=(tag,))
        ...
        try:
            ...
            with db_session() as conn:
                ...
                if metric_key == "total_value":
                    sql = "SELECT symbol, (h.qty * COALESCE(m.current_price, h.avg_price)) as val FROM holdings h LEFT JOIN marketdata m USING(symbol) WHERE broker = ? AND (h.qty * COALESCE(m.current_price, h.avg_price)) > 0 ORDER BY val DESC"
                elif metric_key == "total_invested":
                    sql = "SELECT symbol, (qty * avg_price) as val FROM holdings WHERE broker = ? AND (qty * avg_price) > 0 ORDER BY val DESC"
                elif metric_key == "overall_pnl":
                    sql = "SELECT symbol, running_pnl as val FROM holdings WHERE broker = ? AND running_pnl != 0 ORDER BY val DESC"
                elif metric_key == "unrealized_pnl":
                    sql = "SELECT symbol, (h.qty * COALESCE(m.current_price, h.avg_price) - h.qty*h.avg_price) as val FROM holdings h LEFT JOIN marketdata m USING(symbol) WHERE broker = ? AND (h.qty * COALESCE(m.current_price, h.avg_price) - h.qty*h.avg_price) != 0 ORDER BY val DESC"
                elif metric_key == "unrealized_loss":
                    sql = "SELECT symbol, (h.qty * COALESCE(m.current_price, h.avg_price) - h.qty*h.avg_price) as val FROM holdings h LEFT JOIN marketdata m USING(symbol) WHERE broker = ? AND (h.qty * COALESCE(m.current_price, h.avg_price) - h.qty*h.avg_price) < 0 ORDER BY val ASC"
                elif metric_key == "realized_pnl":
                    sql = "SELECT symbol, realized_pnl as val FROM holdings WHERE broker = ? AND realized_pnl != 0 ORDER BY val DESC"
                elif metric_key == "realized_loss":
                    sql = "SELECT symbol, realized_pnl as val FROM holdings WHERE broker = ? AND realized_pnl < 0 ORDER BY val ASC"
                    ...
                    for broker, total_v in rows:
                        ...
                        for sym, val in cur.fetchall():
                            if abs(val) > 0.01:
                                ...
                                tv.insert(broker, "end", values=(f"   ↳ {sym}", sym_disp, sym_pct_str))
        except Exception as e:
